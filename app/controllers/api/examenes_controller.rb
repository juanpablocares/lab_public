	...
	def show
		if @results = Examen.find(params[:id])
			render json: {
		          ...
		          examen: @results,
		        }, status: 200, include: [:tarifas_examen, :indicacion, :tipo_examen, :tipo_muestra, :alias_examenes, :horaproceso_examenes, :proceso_examen, :procesador_examen]
		else
			render json: {
		          success: false,
